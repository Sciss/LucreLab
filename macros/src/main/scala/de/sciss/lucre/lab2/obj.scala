...
class obj(id: Int) extends StaticAnnotation {
  def macroTransform(annottees: Any*): Any = macro objImpl.impl
}
// cf. https://issues.scala-lang.org/browse/SI-9107
object objImpl {
  ...
  def impl(c: blackbox.Context)(annottees: c.Expr[Any]*): c.Expr[List[c.Tree]] = {
    ...
    val outputs: List[Tree] = inputs match {
      case (cd @ ClassDef(_, cName, _, _)) :: tail =>
        ...
        val cTpe = Ident(TypeName(cName.decodedName.toString))

        val mod0: ModuleDef = tail match {
          case (md @ ModuleDef(_, mName0, mTemp)) :: Nil if cName.decodedName.toString == mName0.decodedName.toString => md
        ...
        val q"$mMods object $mName extends { ..$mEarly } with ..$mParents { $mSelf => ..$mBody }" = mod0
        ...
        val mod1 = q"""$mMods object $mName extends { ..$mEarly } with ..$mParents { $mSelf =>
          ..$mBody
          import de.sciss.lucre.lab2.{Sys, Obj}
          def apply[S <: Sys[S]]()(implicit tx: S#Tx): $cTpe with Obj[S] = ???
          }"""

        // val fooTpt  = TypeTree(cTpe)
        // val fooDef      = q"implicit def $fooName: Foo[$cTpe] = ???"
//        val mTempBody1  = /* fooDef :: */ mApply :: mTempBody0
...
//        val mod1        = ModuleDef(mod0.mods, mod0.name, mTemp1)
    ...
    // println(outputs.map(show(_)).mkString("\n"))
